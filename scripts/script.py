...
if os.getcwd().split(os.sep)[-1] != "coco2yolo":  # Check if current directory is "coco2yolo"
    if not os.path.exists("coco2yolo"):
        os.makedirs("coco2yolo", exist_ok=True)
    os.chdir("coco2yolo")  # Change to the newly created directory
...
for img in coco_data['images']:
    ...
    dest_img_path = f"./train/images/{img_filename}"
    os.system(f"cp {src_img_path} {dest_img_path}")

    # Create label file in YOLO format
    label_filename = os.path.splitext(img_filename)[0] + '.txt'
    label_file_path = f"./train/labels/{label_filename}"
